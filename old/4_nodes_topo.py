...
def run():
    ...
    # nodeA.cmd("route add -host %s gw %s dev nodeA-eth1" % (nodeC_eth1, nodeB_eth1))
    # nodeA.cmd("route add -host %s gw %s dev nodeA-eth1" % (nodeC_eth2, nodeB_eth1))
    # nodeA.cmd("route add -host %s gw %s dev nodeA-eth1" % (nodeD_eth1, nodeB_eth1))

    # nodeB.cmd("route add -host %s dev nodeB-eth1" % nodeA_eth1)
    # nodeB.cmd("route add -host %s dev nodeB-eth2" % nodeC_eth1)
    # nodeB.cmd("route add -host %s dev nodeB-eth2" % nodeC_eth2)
    # nodeB.cmd("route add -host %s gw %s dev nodeB-eth2" % (nodeD_eth1, nodeC_eth2))

    # nodeC.cmd("route add -host %s dev nodeC-eth1" % nodeD_eth1)
    # nodeC.cmd("route add -host %s dev nodeC-eth2" % nodeB_eth1)
    # nodeC.cmd("route add -host %s dev nodeC-eth2" % nodeB_eth2)
    # nodeC.cmd("route add -host %s gw %s dev nodeB-eth2" % (nodeA_eth1, nodeB_eth2))

    # nodeD.cmd("route add -host %s gw %s dev nodeD-eth1" % (nodeB_eth1, nodeC_eth1))
    # nodeD.cmd("route add -host %s gw %s dev nodeD-eth1" % (nodeB_eth2, nodeC_eth1))
    # nodeD.cmd("route add -host %s gw %s dev nodeD-eth1" % (nodeA_eth1, nodeC_eth1))
